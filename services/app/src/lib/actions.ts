...
export async function importProjectData(projectId: string, data: any) {
    ...
    for (const n of nodes) {
        ...
        if (n.id) {
            nodeRefMap.set(n.id, createdNode.id);
        }
        // Always map title to ID to support name-based linking
        nodeRefMap.set(n.title, createdNode.id);
    }

    // 4. Create Links (Parent-Child)
    for (const n of nodes) {
        if (n.children && n.children.length > 0) {
            ...
            for (const childRef of n.children) {
                // Try finding by ID reference first, then by Title
